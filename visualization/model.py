...
class GrowableLLM(nn.Module):
    ...
    # =====================================================
    # 【核心3】小乐协议：无回放的纯融合碎片整理
    # =====================================================
    def defrag(self, optimizer, fusion_data):
        print("🌀 触发 [无回放] 碎片整理与特征对齐...")
        
        # 1. 🌟 全局防御性锁死：先确保所有门都关严
        for param in self.parameters():
            param.requires_grad = False
            
       # 2. 🌟 精准开锁：只解锁最后 6 层（V3 Attention Defrag 协议）
        unlock_start_layer = max(0, len(self.blocks) - 6)
        
        for i in range(unlock_start_layer, len(self.blocks)):
            block = self.blocks[i]
            # 必须用 named_parameters 才能按名字过滤
            for name, param in block.named_parameters():
                # 核心手术刀：只解锁包含 'attn' 或 'norm' 的参数，绝对锁死高层 FFN！
