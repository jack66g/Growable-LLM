...
class DynamicSwiGLU(nn.Module):
    ...
    def expand(self, extra_dim=512):
        """
        原地扩展权重矩阵。前向恒等（gate/up 新行 → 零），
        🔧 同时 down_proj 新列用随机初始化，打破反向梯度死锁。
        """
        ...
        # --- gate_proj: [ffn_dim, hidden_dim] -> [new_dim, hidden_dim] ---
        old_gate_w = self.gate_proj.weight.data
        new_gate_w = torch.zeros(new_dim, self.hidden_dim, device=device, dtype=dtype)
        new_gate_w[:old_dim] = old_gate_w
        # 新增行保持零 → 前向 new_hidden=0，恒等扩容 ✓
        self.gate_proj.weight = nn.Parameter(new_gate_w)
        self.gate_proj.out_features = new_dim

        # --- up_proj: 同上 ---
        old_up_w = self.up_proj.weight.data
        new_up_w = torch.zeros(new_dim, self.hidden_dim, device=device, dtype=dtype)
        new_up_w[:old_dim] = old_up_w
        self.up_proj.weight = nn.Parameter(new_up_w)
        self.up_proj.out_features = new_dim

        # --- down_proj: [hidden_dim, ffn_dim] -> [hidden_dim, new_dim] ---
        old_down_w = self.down_proj.weight.data
        new_down_w = torch.zeros(self.hidden_dim, new_dim, device=device, dtype=dtype)
        new_down_w[:, :old_dim] = old_down_w

        # 🔧 关键修复：新列用随机初始化（小方差），打破梯度死锁。
        #   前向: new_out = down_new_cols @ 0 = 0 → 恒等保持 ✓
        #   反向: ∂loss/∂new_hidden = ∂loss/∂out × down_new_cols^T ≠ 0
        #         → 梯度穿透到 gate/up 新行 → 第一步训练后 new_hidden 开始非零 ✓
        nn.init.normal_(new_down_w[:, old_dim:], std=0.02)
        self.down_proj.weight = nn.Parameter(new_down_w)
