...
@dataclass
class ModelConfig:
    vocab_size: int = 151646    # Qwen 真实词表大小
    hidden_dim: int = 768       # 真实隐藏层维度
    num_layers: int = 12        # 真实层数
    num_heads: int = 12         # 真实注意力头数
    num_kv_heads: int = 4       # 真实 KV 头数
    initial_ffn_dim: int = 3072 # 真实前馈网络维度
...
class GrowableLLM(nn.Module):
    ...
    def defrag(self, optimizer):
        ...
        for block in self.blocks:
            ...
            block.attn_norm.weight.requires_grad = True
            block.ffn_norm.weight.requires_grad = True
